...
export const defaultSelectSecure = {
  id: true,
  email: true,
  emailVerified: true,
  phoneNumber: true,
...
export const getUserPinDigest = async (email: string) => {
  let user = await prisma.user.findUnique({
    where: { email },
    select: defaultSelectSecure,
  });
  ...
  const pinDigest = user?.pin;
  // @ts-expect-error
  if (user) { delete user.passwordDigest, delete user.pin; }
  return { user, pinDigest };
};
